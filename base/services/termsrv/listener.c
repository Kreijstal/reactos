    ...
    _Out_writes_(MAX_PATH) WCHAR *Root)
{
    ...
    if (Length != 0 && Length < ARRAYSIZE(TempPath))
    {
        ...
        if (TermSrvCreateIncomingRoot("cliprdr GetTempPath root create", Root))
            return TRUE;
    ...
    Length = GetWindowsDirectoryW(WindowsPath, ARRAYSIZE(WindowsPath));
    if (Length == 0 || Length >= ARRAYSIZE(WindowsPath))
    {
        TermSrvLogWin32Failure("cliprdr GetWindowsDirectory", GetLastError());
        return FALSE;
    }
    ...
    if (TermSrvCreateIncomingRoot("cliprdr Windows temp root create", Root))
        return TRUE;
    ...
    for (Drive = L'C'; Drive <= L'Z'; Drive++)
    {
        ...
        if (TermSrvCreateIncomingRoot("cliprdr drive temp root create", Root))
        {
            return TRUE;
        }
    }
