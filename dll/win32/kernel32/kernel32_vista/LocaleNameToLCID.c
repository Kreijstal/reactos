    ...
    _In_ LCID Lcid)
{
    ...
    switch (Lcid)
    {
        ...
        case 0x07C1A: return 0x0241A; // "sr" -> "sr-Latn-RS"
    ...
    {
        case LANG_SPANISH:
            return MAKELCID(MAKELANGID(LANG_SPANISH, SUBLANG_SPANISH_MODERN), SORT_DEFAULT);

        case LANG_IRISH:
            return MAKELCID(MAKELANGID(LANG_IRISH, SUBLANG_IRISH_IRELAND), SORT_DEFAULT);

        case LANG_CHINESE:
            return MAKELCID(MAKELANGID(LANG_CHINESE_SIMPLIFIED, SUBLANG_CHINESE_SIMPLIFIED), SORT_DEFAULT);
    }

    if (SUBLANGID(LangId) == SUBLANG_NEUTRAL)
        return MAKELCID(MAKELANGID(PRIMARYLANGID(LangId), SUBLANG_DEFAULT), SORT_DEFAULT);
    ...
    _In_ DWORD dwFlags)
{
    ...
    if ((dwFlags & LOCALE_ALLOW_NEUTRAL_NAMES) == 0)
    {
        ...
        if (SubLangId == SUBLANG_NEUTRAL)
        {
            ...
            Lcid = GetDefaultSpecificLcid(Lcid);
        }
        ...
        if (Lcid == 0)
        {
            DPRINT1("Unhandled neutral LCID %x\n", Lcid);
            SetLastError(ERROR_INVALID_PARAMETER);
            return 0;
        }
    }
