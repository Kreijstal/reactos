...
PITEMID_CHILD ILCreateNetConnectItem(INetConnection * pItem)
{
    ...
    /* An adapter whose Connection key carries no Name - one that was never
     * run through the network class installer - must not take the shell
     * down with it. */
    ...
    /* Ask NDIS what medium the adapter really is; the shell folder uses it
     * to offer the wireless-network list on 802.11 connections. */
    pnetid->SubMediaType = NcIsWirelessAdapter(pProperties->guidId) ? NCSM_WIRELESS
                         : (pProperties->MediaType == NCM_LAN ? NCSM_LAN : NCSM_NONE);
