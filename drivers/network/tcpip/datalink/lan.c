...
/* Use the standard NDEBUG / DPRINT pattern for the LANTransmit/NdisSend
 * trace prints below.  They fire on every IP packet and dominate the
 * kernel debug log; useful for NDIS bring-up but pure noise otherwise.
 * Comment out the "#define NDEBUG" to re-enable. */
#define NDEBUG
#undef UNIMPLEMENTED
...
VOID LanReceiveWorker( PVOID Context ) {
    ...
    if (wc <= 5)
        DPRINT("TCPIP-RX: LanReceiveWorker #%ld\n", wc);
    ...
    if (wc <= 5)
        DPRINT("TCPIP-RX: LanReceiveWorker #%ld PacketType=0x%04x TotalSize=%lu\n",
    ...
    switch (PacketType) {
        case ETYPE_IPv4:
        case ETYPE_IPv6:
            if (wc <= 5) DPRINT("TCPIP-RX: → IPReceive\n");
            IPReceive(Adapter->Context, &IPPacket);
            break;
        case ETYPE_ARP:
            if (wc <= 5) DPRINT("TCPIP-RX: → ARPReceive\n");
            ARPReceive(Adapter->Context, &IPPacket);
            break;
        default:
            if (wc <= 5) DPRINT("TCPIP-RX: dropped, unknown ether type\n");
            IPPacket.Free(&IPPacket);
            break;
    }
}
 ...
 */
{
    ...
    DPRINT("TCPIP-LAN: LANTransmit Adapter=%p State=%d Type=%d LinkAddr=%p\n",
             Adapter, Adapter->State, Type, LinkAddress);

    if (Adapter->State != LAN_STATE_STARTED) {
        DPRINT("TCPIP-LAN: ABORT - Adapter not started (state=%d)\n", Adapter->State);
	...
	DPRINT("TCPIP-LAN: NdisSend XmitPacket=%p Size=%u Handle=%p\n",
	...
	TI_DbgPrint(MID_TRACE, ("NdisSend %s\n",
				...
				"Pending" : "Complete"));
	DPRINT("TCPIP-LAN: NdisSend returned 0x%08x\n", NdisStatus);
