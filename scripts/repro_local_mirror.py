...
def main():
    ...
    s1, _ = run_pacman(c, "step1 -Syu",
                       ["C:\\usr\\bin\\pacman.exe", "-Syu",
                        "--noconfirm"],
                       idle_s=120, deadline_s=600)
    if s1 != "exit":
        print(f"[repro] step1 didn't exit cleanly: {s1}; bailing", flush=True)
        q.kill()
        sys.exit(1)

    argv = ["C:\\usr\\bin\\pacman.exe", "-S",
            "--noconfirm"] + list(pkgs)
    s2, info = run_pacman(c, "step2 install", argv,
                          idle_s=900, deadline_s=7200)
    print(f"[repro] step2 result = {s2}")

    # Drain: run sync.exe and wait up to 300 s for it to complete.
    drain_deadline = time.time() + 300
    try:
        c.spawn("C:\\usr\\bin\\sync.exe", "C:\\usr\\bin\\sync.exe",
                timeout_ms=300000, idle_timeout_ms=300000)
        while time.time() < drain_deadline:
            st, _ = c.stream(deadline=min(drain_deadline, time.time() + 5),
                             on_stdout=lambda p: None,
                             on_stderr=lambda p: None, echo=False)
            if st in ("exit", "eof"):
                print(f"\n[repro] sync {st}", flush=True)
                break
    except Exception:
        pass
    # Give the lazy writer time to drain any remaining dirty pages.
    remaining = drain_deadline - time.time()
    if remaining > 0:
        print(f"[repro] extra drain {remaining:.0f}s", flush=True)
