/*
 * PROJECT:     ReactOS SDK
 * LICENSE:     Zope Public License 2.1 (GPL-compatible per FSF)
 * PURPOSE:     Native 802.11 (DOT11_*) types and OID constants used by
 *              NDIS 6 wireless miniports, wlansvc, and wlanapi.dll.
 *
 * Ported wholesale from the mingw-w64 runtime package (Copyright (c) 2009
 * 2010 by the mingw-w64 project, ZPL 2.1).  Bringing the full surface in
 * lets ReactOS write a real native-802.11 miniport (e.g. AR9485) without
 * inventing its own type system.  ZPL 2.1 is GPL-2.0+ compatible per the
 * FSF; redistribution must preserve this attribution and the disclaimer
 * notice that mingw-w64 ships in its DISCLAIMER file.
 */
