...
function(add_delay_importlibs _module)
    ...
    foreach(_lib ${ARGN})
        ...
        if(ARCH STREQUAL "arm64")
            # llvm-dlltool cannot create delay-load import libraries, so link
            # the regular import library and have LLD turn its imports into
            # delay-loads (resolved through delayimp's __delayLoadHelper2).
            get_filename_component(_ext "${_lib}" EXT)
            if(NOT _ext)
                set(_ext ".dll")
            endif()
            target_link_libraries(${_module} lib${_basename})
            target_link_options(${_module} PRIVATE "LINKER:--delayload=${_basename}${_ext}")
        else()
