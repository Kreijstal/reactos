...
/*
 * Pre-Windows-8 fallback.
 *
 * The implementation above blocks in NtWaitForAlertByThreadId, which ntdll
 * only exports from Windows 8 on (-version=0x602+ in ntdll.spec).  Below that
 * these three functions still have to exist: ntdll_vista.dll is the
 * compatibility shim that hands Vista-and-later APIs to a lower-versioned
 * system, and d3dx10_43's thread pump calls RtlWaitOnAddress directly.  So
 * keep the original keyed-event rendezvous for those targets -- keyed events
 * are XP-era and ungated.  Its two structural hazards (a waker that blocks
 * until its waiter arrives, and a timed-out waiter that must drain a pending
 * release) are documented inline; they are the reason the Win8 path exists,
 * not a reason to have no path at all here.
 */
