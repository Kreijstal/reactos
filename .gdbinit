# ReactOS AMD64 GDB debug helper for QEMU
# Usage: gdb -x .gdbinit
# Or:    gdb, then "source .gdbinit"
#
# Connects to QEMU gdbstub and loads symbols for ntoskrnl + drivers.
# Requires: Python support in GDB (standard in modern GDB).
...
def read_unicode_string(addr):
    ...
    buf_ptr = read_u64(addr + 8)  # 2+2+4(pad on x64) = 8
...
def find_pe_base_from_rip():
    ...
    # Only search in kernel space
    if rip < KSEG0_BASE:
        return None

    # Search backwards from RIP, page-aligned
    page = rip & ~0xFFF
    for _ in range(4096):  # up to 16MB back
        sig = read_u16(page)
        if sig == 0x5A4D:  # 'MZ'
            # Verify PE signature
...
def get_pe_section_rva(base, name):
    """Get the RVA of a named section from a loaded PE."""
    pe_off = read_u32(base + 0x3C)
    if not pe_off:
        return None
    # COFF header is at pe_off + 4
    num_sections = read_u16(base + pe_off + 6)
    size_opt = read_u16(base + pe_off + 20)
    if num_sections is None or size_opt is None:
        return None
    # Sections start after optional header
    ...
    for i in range(num_sections):
        ...
        if sec_name == name:
            return read_u32(sec + 12)  # VirtualAddress
    return None
...
class ReactosLoadSymbols(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        # Strategy 1: Try the known ImageBase (0x400000) relocated to KSEG0
...
class ReactosModuleList(gdb.Command):
    ...
    Optionally load symbols for all of them if build paths can be resolved.
    ...
    def invoke(self, arg, from_tty):
        ...
        # First we need PsLoadedModuleList address.
        # Try to read it as a symbol (requires ntoskrnl symbols loaded first).
        try:
        ...
        # LIST_ENTRY: Flink at offset 0, Blink at offset 8
        flink = read_u64(list_head)
        if flink is None:
            print("Cannot read PsLoadedModuleList")
            return

        # LDR_DATA_TABLE_ENTRY layout (amd64):
        #   +0x00 InLoadOrderLinks (LIST_ENTRY: 16 bytes)
        #   +0x10 InMemoryOrderLinks (16 bytes)
        #   +0x20 InInitializationOrderLinks (16 bytes)
        #   +0x30 DllBase (8 bytes)
        #   +0x38 EntryPoint (8 bytes)
        #   +0x40 SizeOfImage (4 bytes)
        #   +0x48 FullDllName (UNICODE_STRING: 16 bytes on x64)
        #   +0x58 BaseDllName (UNICODE_STRING: 16 bytes on x64)

        print(f"{'Base':>20s}  {'Size':>10s}  Name")
        print(f"{'----':>20s}  {'----':>10s}  ----")

        # Known build paths for common modules
        MODULE_PATHS = {
...
# KTHREAD field offsets for ReactOS amd64 (empirically verified)
KTHREAD_INITIAL_STACK = 0x28
KTHREAD_KERNEL_STACK  = 0x38
KTHREAD_APC_STATE     = 0x48  # KAPC_STATE embedded here
KTHREAD_KERNEL_APC_DISABLE = 0x1B4
KTHREAD_SPECIAL_APC_DISABLE = 0x1B2

# KAPC_STATE offsets (relative to KAPC_STATE base)
KAPC_STATE_KERNEL_LIST   = 0x00  # LIST_ENTRY ApcListHead[0]
KAPC_STATE_USER_LIST     = 0x10  # LIST_ENTRY ApcListHead[1]
...
# KWAIT_BLOCK offsets
KWAIT_BLOCK_WAIT_LIST_ENTRY = 0x00
KWAIT_BLOCK_THREAD          = 0x10
KWAIT_BLOCK_OBJECT          = 0x18

# KAPC offsets (ApcListEntry is what's linked in the list)
KAPC_APC_LIST_ENTRY    = 0x10  # LIST_ENTRY
KAPC_KERNEL_ROUTINE    = 0x20
KAPC_RUNDOWN_ROUTINE   = 0x28
KAPC_NORMAL_ROUTINE    = 0x30
KAPC_NORMAL_CONTEXT    = 0x38
KAPC_SYSTEM_ARG1       = 0x40
KAPC_SYSTEM_ARG2       = 0x48

def get_module_map():
    """Build a map of address ranges to module names from PsLoadedModuleList."""
    modules = []
    try:
        list_head = int(gdb.parse_and_eval("(unsigned long long)&PsLoadedModuleList"))
    except:
    ...
    entry = flink
    count = 0
    while entry != list_head and count < 256:
        base = read_u64(entry + 0x30)
        size = read_u32(entry + 0x40)
        name = read_unicode_string(entry + 0x58)
        if base and size:
            modules.append((base, base + size, name or "?"))
...
def addr_to_sym(addr):
    ...
    try:
        ...
        if "No symbol" not in sym:
            # Trim the verbose "in section .text of /long/path" part
...
def dump_thread_stack(thread_addr):
    ...
    for off in range(0, min(init_stack - kern_stack, 0x1000), 8):
        ...
        if val is None:
            continue
        # Check if it looks like a kernel/driver code address
...
class ReactosFindWaiters(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        # DISPATCHER_HEADER:
        #   +0x00: Type (1), Absolute (1), Size (1), Inserted (1)
        #   +0x04: SignalState (4)
        #   +0x08: WaitListHead (LIST_ENTRY, 16)
        ...
        blink = read_u64(wait_list_head + 8)
        ...
        while entry != wait_list_head and count < 32:
            # entry points to KWAIT_BLOCK.WaitListEntry
...
class ReactosIrpInfo(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        io_stat  = read_u32(irp + 0x30)  # IoStatus.Status
        io_info  = read_u64(irp + 0x38)  # IoStatus.Information
        user_iosb = read_u64(irp + 0x48)
        user_event = read_u64(irp + 0x50)
        cancel_routine = read_u64(irp + 0x60)
        user_buffer = read_u64(irp + 0x68)
        thread   = read_u64(irp + 0x78)  # Tail.Overlay.Thread

        flag_names = []
        if flags:
            if flags & 0x01: flag_names.append("NOCACHE")
            if flags & 0x02: flag_names.append("PAGING_IO")
            if flags & 0x04: flag_names.append("MOUNT_COMPLETION")
            if flags & 0x08: flag_names.append("SYNCHRONOUS_API")
            if flags & 0x10: flag_names.append("INPUT_OPERATION")
            if flags & 0x20: flag_names.append("DEFER_IO_COMPLETION")
            if flags & 0x40: flag_names.append("SYNCHRONOUS_PAGING_IO")
            if flags & 0x80: flag_names.append("CREATE_OPERATION")
            if flags & 0x100: flag_names.append("READ_OPERATION")
            if flags & 0x200: flag_names.append("WRITE_OPERATION")
            if flags & 0x400: flag_names.append("CLOSE_OPERATION")
