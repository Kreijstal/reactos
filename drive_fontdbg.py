#!/usr/bin/env python3
"""Reinstall with the FONTDBG-instrumented ntfs.sys, boot from HD, and capture
the NtfsReadFile paging-read trace for *.ttf so we can see whether font glyph
...
NAME = "nt10fontdbg"; ISO = "build_nt10/bootcd.iso"
...
def main():
    ...
    # Capture FONTDBG + zero-glyph evidence
    lines = q.log_lines()
    fontdbg = [l.strip() for l in lines if "FONTDBG" in l]
    zglyph  = sum(1 for l in lines if "glyphSize.cx of zero" in l)
    print(f"=== FONTDBG lines: {len(fontdbg)} ; zero-glyph warnings: {zglyph} ===", flush=True)
